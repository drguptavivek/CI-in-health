...
Based on CISA sector documents and advisories emphasize as **mission-essential functions**, **data dependence**, and **operational disruption patterns**, the “highest criticality” set in health typically clusters into these categories:
...
1. **CISA — Critical Infrastructure Sectors (PDF)** (includes Healthcare and Public Health as a sector).
2. **CISA — Healthcare and Public Health Sector (PDF of CISA page)** (sector overview framing).
3. **NIPP Sector-Specific Plan: Healthcare and Public Health (2015)** — dependency on complex IT and rapid/secure data flows. 
4. **CISA HPH Sector Mitigation Guide (TLP:CLEAR)** — emphasizes protection of critical HPH systems/functions/data and patient-focused impacts.
5. **CISA — CIRCIA Fact Sheet (2022)** — 72-hour covered incident reporting and 24-hour ransomware payment reporting timelines. 
6. **CISA — CIRCIA NPRM Overview (May, PDF)** — proposed-rule layer for defining covered entities/incidents (scoping).
7. **AA20-302A — Ransomware Activity Targeting the HPH Sector (PDF)** — operational disruption framing for HPH ransomware.
8. **AA23-349A — HPH Sector Risk & Vulnerability Assessment (PDF)** — internal weaknesses leading to domain compromise; recommended mitigations (phishing-resistant admin MFA, default credentials removal, segregation).
